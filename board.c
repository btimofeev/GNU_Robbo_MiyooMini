...
update_game(void)
{
	...
	for (x = 0; x < level.w; x++) {	/* Thunor: I have swapped these around to process by rows */
		 ...
		 * their delays here 
		 */
		if (board[x][y].type == GUN || board[x][y].type == BIRD || board[x][y].type==MAGNET) {
		...
		if (board[x][y].moved <= 0) {
			...
			if (board[coords.x][coords.y].shooted == 0) {
				...
				if (((rand()) & 0x07) == 0) {	/* it's blaster */
				    ...
				    case BOMB2:
				    case BOMB:
					board[coords_temp. x][coords_temp.y].blowed = DELAY_LASER;
					break;
				    default:
...
blow_bomb(int x, int y)
{
	...
	if (!coords_out_of_range(coords) && !board[coords.x][coords.y].blowed && board[coords.x][coords.y].blowable) {
		...
		if (board[coords.x][coords.y].solidlaser
			...
			board[laserpath.x][laserpath.y].moved = DELAY_BOMB_TARGET;
		    }
		}
		/*
